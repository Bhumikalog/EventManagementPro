...
export default function MyRegistrations() {
  ...
  // ✅ Load only after user ID is ready
  useEffect(() => {
    if (user?.id) loadRegistrations();
  }, [user?.id]);
  ...
  const loadRegistrations = async () => {
    setLoading(true);
    try {
      console.log("🎯 Querying with user_id:", user?.id);
  ...
  // ✅ Cancel registration (via Edge Function)
                ...
                {(reg.registration_status === 'confirmed' || reg.registration_status === 'waitlisted') && (
                  <Button
                    ...
                    onClick={() => {
                      ...
                      handleCancel(reg.id, reg.event_id || reg.events?.id);
                    }}
                  >
                    Cancel
                  </Button>
